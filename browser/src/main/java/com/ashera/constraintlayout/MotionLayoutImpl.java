...
return layoutParams.wrapBehaviorInParent;			}
	...
	public class MotionLayoutExt extends androidx.constraintlayout.motion.widget.MotionLayout implements ILifeCycleDecorator{
    	...
    	public r.android.view.View inflateView(java.lang.String layout) {
    		...
    		IWidget widget = template.loadLazyWidgets(MotionLayoutImpl.this.getParent());
...
private void nativeAddOnSwipe(androidx.constraintlayout.motion.widget.MotionScene.Transition transition, IWidget widget) {
	org.teavm.jso.dom.events.EventListener<ViewImpl.HtmlMouseEvent> flistener = (event) -> {
		...
		case "mousedown":
			action = 1;
			processTouchEvent(transition, MotionEvent.ACTION_DOWN, event.getClientX(), event.getClientY());
			break;
		case "mouseup":
			if (action == 1) {
				processTouchEvent(transition, MotionEvent.ACTION_UP, event.getClientX(), event.getClientY());
				action = 0;
			}
			break;
		case "mousemove":
			if (action == 1) {
				processTouchEvent(transition, MotionEvent.ACTION_MOVE, event.getClientX(), event.getClientY());
			}
			break;
		default:
			break;
	...
	ViewImpl.setOnListener(widget, fragment.getRootWidget().asNativeWidget(), flistener, "mousemove", "mousemove");
...
class DallocHandler extends com.ashera.widget.bus.EventBusHandler {
	...
	protected void doPerform(Object payload) {
		ViewImpl.removeListener(MotionLayoutImpl.this, org.teavm.jso.browser.Window.current(), eventId, "mouseup");
	}
