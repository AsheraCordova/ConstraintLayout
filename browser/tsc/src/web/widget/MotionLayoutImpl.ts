...
export abstract class MotionLayoutImpl<T> extends ViewGroupImpl<T>{
	//start - body
	static initialize() {
    }	
	@Type(() => CommandAttr)
	...
	public setLayoutDescription(value : string) : T {
		this.resetIfRequired();
		if (this.layoutDescription == null || this.layoutDescription == undefined) {
			this.layoutDescription = new CommandAttr<string>();
		}
		
		this.layoutDescription.setSetter(true);
		this.layoutDescription.setValue(value);
		this.orderSet++;
		this.layoutDescription.setOrderSet(this.orderSet);
		return this.thisPointer;
	}
		

	public tryGetProgress() : T {
		this.resetIfRequired();
		if (this.progress == null || this.progress == undefined) {
			this.progress = new CommandAttr<number>()
		}
		
		this.progress.setGetter(true);
		this.orderGet++;
		this.progress.setOrderGet(this.orderGet);
		return this.thisPointer;
	}
	...
	public getProgress() : number {
		...
		return this.progress.getCommandReturnValue();
	}
...
export class MotionLayout_LayoutParams extends MotionLayoutImpl_LayoutParams<MotionLayout_LayoutParams> implements ILayoutParam {
    getThisPointer(): MotionLayout_LayoutParams {
        return this;
    }
...
export class MotionLayout extends MotionLayoutImpl<MotionLayout> implements IWidget{
    getThisPointer(): MotionLayout {
        return this;
    }
