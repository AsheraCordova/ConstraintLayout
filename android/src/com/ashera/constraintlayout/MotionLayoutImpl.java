...
return getOptimizationLevel();			}
	...
	private void setLayoutDescriptionExternal(Object objValue) {
			...
			public void startElement(String uri, String localName, String qName, org.xml.sax.Attributes atts) throws org.xml.sax.SAXException {
				...
				case "ConstraintSet": {
					...
					if (id != null) {
						...
						if (derivedId != null) {
							putDerivedId(constraintSetId, getId(derivedId));
							copyDerivedContraints(constraintSet, constraintSetId, motionLayout);
						}
					}
					break;
				}
				case "CustomAttribute": {
					...
					for (int i = 0; i < atts.getLength(); i++) {
						...
						case "customColorValue":
							value = android.graphics.Color.parseColor(attrValue);
					...
					if (id != null) {
						...
						if (atts.getLength() > 1) {
							...
							if (constraint != null) {
								...
								androidx.constraintlayout.widget.Barrier barrier = new androidx.constraintlayout.widget.Barrier((Context) fragment.getRootActivity());
								
								parseConstraint(MotionLayoutImpl.this, barrier, constraint.motion, constraint.propertySet, layoutParams, atts);
								fillFromConstraints(constraint, barrier, constraintId, layoutParams);
							}
						}
					}
					break;
				...
				case "OnClick": {
					...
					if (clickAction != -1) {
						...
						int mConstraintSetStart = (int) getFieldValueUsingReflection(transition, "mConstraintSetStart");
						setFieldUsingReflection(transition, "mConstraintSetStart", -1);
						clickListener.addOnClickListeners(motionLayout, motionLayout.getCurrentState(), transition);
						setFieldUsingReflection(transition, "mConstraintSetStart", mConstraintSetStart);
					}
				...
				case "KeyTrigger": {
					...
					Map mCustomConstraints1 = new HashMap();mCustomConstraints1.putAll(mCustomConstraints);setFieldUsingReflection(keyTrigger, "mCustomConstraints", mCustomConstraints1);
				...
				case "KeyCycle": {
					...
					for (int i = 0; i < atts.getLength(); i++) {
						...
						case "waveShape":
							...
							break;
						case "waveVariesBy":
							int mWaveVariesBy = (int) getWaveVariesBy(value);setFieldUsingReflection(keyCycle, "mWaveVariesBy", mWaveVariesBy);
							break;
						case "wavePhase":
							keyCycle.setValue(name, (float) quickConvert(value, "float"));
							break;
...
int mPathMotionArc = getPathMotionArc(strValue);setFieldUsingReflection(keyPosition, "mPathMotionArc", mPathMotionArc);
...
int mConstraintSetStart = (int) w.quickConvert(strValue, "id");setFieldUsingReflection(transition, "mConstraintSetStart", mConstraintSetStart);
}
...
int mConstraintSetEnd = (int) w.quickConvert(strValue, "id");setFieldUsingReflection(transition, "mConstraintSetEnd", mConstraintSetEnd);
}
...
private void setCurveFit(IWidget w, androidx.constraintlayout.motion.widget.KeyPosition keyPosition, String strValue) {
	int mCurveFit = getCurveFit(strValue);setFieldUsingReflection(keyPosition, "mCurveFit", mCurveFit);
}
...
private void setFramePosition(IWidget w, androidx.constraintlayout.motion.widget.KeyTrigger keyTrigger, String strValue) {
	...
	float mFireThreshold = (val + .5f) / 100f;setFieldUsingReflection(keyTrigger, "mFireThreshold", mFireThreshold);
}
...
//end - postSetAttribute
	...
	private void setLayoutDescription(Object objValue) {
		...
		if (fragment.getRootDirectory() == null && inlineResource == null) {
			...
			String xml = value.replaceAll("@xml/", "");
			...
			setLayoutDescriptionExternal(objValue);
		}
	...
	private void copyDerivedContraints(androidx.constraintlayout.widget.ConstraintSet cs, int key, androidx.constraintlayout.motion.widget.MotionLayout motionLayout) {
		cs.derivedState = cs.mIdString;
		 android.util.SparseIntArray mDeriveMap = (android.util.SparseIntArray) getFieldValueUsingReflection(motionLayout.getScene() , "mDeriveMap");
		 int derivedFromId = mDeriveMap.get(key);
		if (derivedFromId > 0) {
			android.util.SparseArray<androidx.constraintlayout.widget.ConstraintSet> mConstraintSetMap = (android.util.SparseArray<androidx.constraintlayout.widget.ConstraintSet>) getFieldValueUsingReflection(motionLayout.getScene() , "mConstraintSetMap");
			androidx.constraintlayout.widget.ConstraintSet derivedFrom = mConstraintSetMap.get(derivedFromId);
			if (derivedFrom == null) {
				android.util.Log.e("MotionLayout", "ERROR! invalid deriveConstraintsFrom: @id/" + derivedFromId);
				return;
			}
	...
	private void putDerivedId(int id,int derivedId){
		android.util.SparseIntArray mDeriveMap = (android.util.SparseIntArray) getFieldValueUsingReflection(motionLayout.getScene() , "mDeriveMap");
	...
	private void fillFromConstraints(androidx.constraintlayout.widget.ConstraintSet.Constraint constraint, androidx.constraintlayout.widget.Barrier barrier, int constraintId, androidx.constraintlayout.widget.Constraints.LayoutParams layoutParams) {
		invokePrivateMethodUsingReflection(constraint, "fillFromConstraints",
				new Class[] {androidx.constraintlayout.widget.ConstraintHelper.class, int.class, layoutParams.getClass()},
				barrier, constraintId, layoutParams);
	}
}
